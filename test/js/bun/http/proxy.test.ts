...
describe("proxy resolution", () => {
  ...
  test("which proxy the environment selects", () => {
    ...
    const cases: [env: Record<string, string>, url: string, proxy: string | null][] = [
      ...
      // a SOCKS5 proxy, the usual value of ALL_PROXY, serves both schemes
      [{ ALL_PROXY: "socks5://proxy.test:1080" }, "http://example.test/", "socks5://proxy.test:1080"],
      [{ ALL_PROXY: "socks5h://proxy.test:1080" }, "https://example.test/", "socks5h://proxy.test:1080"],
      // ...unless it names a proxy the client cannot speak to
      [{ ALL_PROXY: "socks4://proxy.test:1080" }, "http://example.test/", null],
      [{ ALL_PROXY: "socks4a://proxy.test:1080" }, "https://example.test/", null],
...
describe.concurrent("proxy environment", () => {
  ...
  test("ALL_PROXY is the fallback for an http(s) proxy, and a socks4 one is left alone", async () => {
    const results = await run(
      () => ({}),
      `
      const out = {};
      ...
      process.env.ALL_PROXY = "socks4://127.0.0.1:1";
      out.socks4 = await via("localhost");
...
describe.concurrent("socks5 proxy", () => {
  ...
  const cleanProxyEnv = { ...bunEnv, ...proxyFreeEnv };
  ...
  for (const variables of ["HTTP_PROXY and HTTPS_PROXY", "ALL_PROXY"]) {
    test(`${variables} accept a socks5:// URL`, async () => {
      using socks = await createSocks5Server();
      using httpOrigin = createEchoOrigin(false);
      using httpsOrigin = createEchoOrigin(true);
      await using proc = Bun.spawn({
        ...
        env: {
          ...
          ...(variables === "ALL_PROXY"
            ? { ALL_PROXY: socks.url() }
            : { HTTP_PROXY: socks.url("SOCKS5"), HTTPS_PROXY: socks.url("socks5h") }),
        },
        stdout: "pipe",
        stderr: "pipe",
      });
