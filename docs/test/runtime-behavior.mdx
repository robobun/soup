...
Only imported files are watched. To rerun the tests when a fixture or another file that nothing imports changes, add [`--watch-path`](/runtime/watch-mode#files-that-are-not-imported):
