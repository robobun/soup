...
A workspace can also point `main`, `types`, `exports` or `bin` at its source files, so that the other workspaces use it without a build step, and list the build output under [`publishConfig`](/pm/cli/publish#publishconfig). `bun publish` and `bun pm pack` put the `publishConfig` values in the published `package.json`.
