...
If the subcommand you type does not exist, `bun pm` prints its help and points at the closest one it has.
...
The `package.json` in the tarball is the one `bun publish` would publish: `workspace:` and `catalog:` versions are resolved, and fields such as `main`, `exports` and `bin` are replaced by their [`publishConfig`](/pm/cli/publish#publishconfig) overrides.
