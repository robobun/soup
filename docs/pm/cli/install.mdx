...
A `bun.lock` with git conflict markers fails `--frozen-lockfile`. Run `bun install` to [merge both sides](/pm/lockfile#merge-conflicts), then commit the lockfile.
...
To require a version of Bun for a project, add a `"bun"` entry to the `"engines"` field of its `package.json`. The value is a semver range, in the same syntax as a dependency version.
...
`bun install` (and `bun add`, `bun remove`, `bun update`) compares the running version with the range before it resolves anything. When the version is outside the range, the command stops with an error and exit code 1, and writes nothing. [`bun run <script>`](/runtime#run-a-package-json-script) makes the same check before it runs a `package.json` script.
...
The check uses the `major.minor.patch` of the running Bun, the value of `Bun.version`. In a workspace, the root `package.json` is checked, from whichever package `bun install` runs in. Only the project's own `package.json` is checked: the `engines` of a dependency are not, and the other entries of `engines` (`node`, `npm`) are ignored. A range that does not parse, such as `"latest"`, is an error too.
