...
🟡 `Worker` ignores the `resourceLimits` and `trackUnmanagedFds` options. Some `execArgv` flags take effect in the worker, for example `--no-addons`, `--stack-trace-limit` and `--tls-min-v1.3`. Others, for example `--conditions` and `--no-deprecation`, only set `process.execArgv`. `worker.performance.eventLoopUtilization()` is a stub. `locks` is implemented; `locks.query()` reports the locks and requests of every thread, as the Web Locks specification says, where Node.js reports only the calling thread's. Missing `moveMessagePortToContext`.
...
🟡 `userAgent`, `platform`, `hardwareConcurrency` and [`locks`](/runtime/workers#locks-with-navigator-locks) are implemented. Missing `language` and `languages`. The `Navigator` class is not a global.
