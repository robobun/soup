...
Under [`bun --watch` and `bun --hot`](/runtime/watch-mode), Bun watches the `.env` files it loaded, including those passed with `--env-file`, and restarts the process when one of them changes.
...
| `BUN_CONFIG_VERBOSE_SQL`                 | If `BUN_CONFIG_VERBOSE_SQL=1` (or `true`), then `Bun.SQL` prints each query and its parameters to stderr before it is sent, as the [`debug: true`](/runtime/sql#logging-queries) option does. It applies to every instance that does not set `debug`.                                                                                                                         |
