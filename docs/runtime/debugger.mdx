...
Set the `BUN_CONFIG_VERBOSE_SQL` environment variable to `1` or `true` to print the queries made with [`Bun.SQL`](/runtime/sql#logging-queries), to PostgreSQL, MySQL and SQLite alike. Each query is printed to stderr before it is sent, followed by its parameters.
...
```txt
[sql] BEGIN
[sql] SELECT * FROM users WHERE id = $1 [ 42 ]
[sql] COMMIT
```

It is the same as passing `debug: true` to each `SQL` instance, and an instance that sets the `debug` option is not affected.
