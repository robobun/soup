...
`.timeout(ms)` kills a command that is still running after `ms` milliseconds. `.signal(abortSignal)` kills it when an `AbortSignal` is aborted.
...
try {
  ...
  console.log(err.exitCode); // 143 if it timed out
  console.log(err.stderr.toString()); // what curl printed until then
}

const controller = new AbortController();
const tests = $`bun test`.nothrow().signal(controller.signal);
controller.abort(); // for example from a "Cancel" button
const { exitCode } = await tests; // 143

// Both come before the output helpers.
const head = await $`git rev-parse HEAD`.timeout(1000).text();
```

`.kill(signal?)` does the same thing directly. A command starts once it is awaited, so keep the promise to have something to call it on, or start the command right away with `.run()`:
...
// Wait for Ctrl+C, then wait for the command to stop.
const ping = $`ping bun.com`.nothrow();
process.on("SIGINT", () => ping.kill());
await ping;
```

All three stop the whole script, not only one command:

- Every process the script is running gets the signal: each member of a pipeline, command substitutions, subshells.
- Nothing else starts. In `` $`sleep 10; echo done` `` the `echo` does not run, nor does the other side of `&&` and `||` or the other branch of an `if`.
- Builtins that are waiting for input see it end, whether that is the script's stdin or a pipe from another command, and `yes` stops. A builtin that is in the middle of work on the file system, such as `rm -rf` or `cp -r`, finishes that first.

The promise settles once those processes have exited, the way it does for any other exit code. The exit code is 128 plus the number of the signal, as in other shells: `143` for `SIGTERM`, `137` for `SIGKILL`. So the promise rejects with a `ShellError` unless `.nothrow()` was called, and `stdout` and `stderr` hold everything written up to that point. A command that is killed before it started runs nothing. Killing one that has finished does nothing.

The signal is `SIGTERM` unless `.kill()` is given another one, by name or by number. `.killSignal(signal)` changes it for `.timeout()` and `.signal()`, and a second `.kill()` can follow up on a process that ignored the first one:
...
const job = $`./ignores-sigterm.sh`.nothrow().run();
job.kill();
job.kill("SIGKILL");
await job; // exit code 143: the first signal decides it
```

Whatever the signal is, `.kill()` ends the script: it is not a way to send `SIGUSR1` or `SIGSTOP` to a command that should go on running. Signals go to the processes the shell started, not to processes those have started themselves. The promise does not wait for such a process even if it still holds the command's stdout or stderr open. On Windows, `SIGTERM`, `SIGKILL`, `SIGINT` and `SIGQUIT` all terminate the process at once, and other signals cannot be sent.
...
`for` runs a list of commands once for each word in a list, with a variable set to that word:
...
// An interpolated array is one word per element, and no element is ever split or globbed
const packages = ["core", "cli", "my docs"];
await $`for pkg in ${packages}; do (cd packages/$pkg && bun run build); done`;
```

On Windows, `bun run` runs `package.json` scripts in Bun Shell, so a loop in a script works there the way it does in `sh` on Linux and macOS:
...
The words after `in` are expanded like the arguments of a command: variables, command substitution (split into words unless it is quoted), brace expansion, globs and interpolated JavaScript values all work. They are expanded once, before the first iteration. Three things to know, all of which are how Bun Shell expands arguments everywhere:

- A variable is one word, even when its value has spaces in it. For a list, interpolate an array or use command substitution: `for f in $(cat files.txt)`.
- The matches of a glob are not sorted.
- A glob that matches nothing is an error: the loop prints `no matches found`, does not run, and exits with `1`.

`break` ends the loop and `continue` skips to the next word. In nested loops both take a count, so `break 2` also ends the loop around the current one:
...
await $`
  for dir in packages/*; do
    ...
    for script in build test; do
      # a script failed: leave both loops
...
The exit code of a loop is the exit code of the last command its body ran, or `0` when the body never ran. `break` and `continue` are commands that succeed, so a loop that ends with one of them exits with `0`. The variable is a shell variable: it is not exported to the commands in the body, and it keeps its last value after the loop. A loop can be a member of a pipeline and an operand of `&&` and `||`.

`while` and `until` loops, `for name; do` without a word list, and redirecting a whole loop (`done > file`) are not supported yet. Redirect the commands in the body instead.
...
`[[ ... ]]` tests a condition and exits with `0` when it holds and `1` when it does not, so it works with `&&`, `||` and `if`:

```js
import { $ } from "bun";

// Rebuild when the bundle is missing or older than its source
await $`[[ dist/index.js -nt src/index.ts ]] || bun build ./src/index.ts --outdir dist`;

const { exitCode } = await $`bun test`.nothrow();
await $`
...
Each `[[ ... ]]` holds a single test; combine several with `&&` and `||` between them. Operands are expanded like command arguments (variables, command substitution and interpolated JavaScript values all work), but the operator itself has to be written literally. The supported tests are:
...
| `-L file`, `-h file`                         | `file` is a symbolic link (the link itself is tested, every other test follows it)       |
| `-p file`                                    | `file` is a named pipe                                                                   |
| `-S file`                                    | `file` is a socket                                                                       |
| `-b file`, `-c file`                         | `file` is a block or character device                                                    |
| `a -nt b`                                    | `a` was modified more recently than `b`, or `a` exists and `b` does not                  |
| `a -ot b`                                    | `a` was modified less recently than `b`, or `b` exists and `a` does not                  |
| `a -ef b`                                    | `a` and `b` are the same file (the same device and inode, also through hard links)       |
| `-z string`, `-n string`                     | `string` is empty, or is not empty                                                       |
| `a == b`, `a = b`, `a != b`                  | the strings are equal, or are not equal                                                  |
| `a -eq b`, `-ne`, `-lt`, `-le`, `-gt`, `-ge` | the integers compare equal, not equal, less, less or equal, greater, or greater or equal |

File tests are relative to the shell's current directory and an empty operand never names a file, so `[[ -e $MAYBE_UNSET ]]` is simply false.

The integer tests take optionally signed decimal integers, with whitespace around them ignored. Like in bash, an empty operand counts as `0`, so an unset variable compares as zero. Any other operand that is not an integer prints `[[: <operand>: integer expression expected` to stderr and the test fails with exit code `1`.
...
- `mv`
- `exit`
- `break`, `continue`: leave a [`for` loop](#loops-for), or skip to its next word
- `true`
- `false`
- `yes`
- `seq`
- `dirname`
- `basename`
- `wc`: count lines, words, bytes (`-l`, `-w`, `-c`) and UTF-8 characters (`-m`) of stdin or files
- `head`: print the first lines (`-n`) or bytes (`-c`) of stdin or files; `-n -N` prints all but the last N
- `tail`: print the last lines (`-n`) or bytes (`-c`) of stdin or files; `-n +K` prints from line K on

`head` and `tail` print a `==> name <==` header before each of several files (`-q` never, `-v` always), and `head` stops reading a pipe as soon as it has what it needs, so `yes | head -n 3` ends.
...
- `sort`: sort the lines of stdin or files: `-r` reverse, `-n` numeric, `-f` ignore case, `-u` drop repeated lines, `-s` stable, `-b` ignore leading blanks, `-k F[.C][OPTS][,G[.C][OPTS]]` sort by a field (with `-t CHAR` as the field separator), `-c` check that the input is sorted, `-o FILE` write to a file (it can be the input), `-z` NUL-terminated lines
- `uniq`: collapse adjacent repeated lines of stdin or a file: `-c` prefix each line with its count, `-d` only repeated lines, `-D` every copy of a repeated line, `-u` only lines that are not repeated, `-i` ignore case, `-f N` skip the first N fields, `-s N` skip N characters, `-w N` compare at most N characters, `-z` NUL-terminated lines; a second operand is the output file

Both follow GNU coreutils: the output is the same byte for byte, `sort -n` reads a line without a number as 0, and keys compare equal under `-u` the way they do in `-k`.

```ts
await $`sort -u names.txt`;
await $`sort -t, -k2 -n data.csv | head -n 10`;
const top = await $`sort visitors.txt | uniq -c | sort -rn | head -n 5`.text(); // the 5 most frequent lines
await $`sort -c versions.txt`; // exit 1 and "sort: versions.txt:3: disorder: ..." when it is not sorted
```

`sort -h`, `-V`, `-R`, `-M`, `-g` and `-m` are not supported.
