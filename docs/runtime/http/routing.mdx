...
<Note>
  To serve a directory without writing a server, run [`bun serve ./public`](/runtime/http/static-server). It adds
  directory listings, clean URLs, a `404.html` page and a single-page app mode, and it does not send dotfiles.
</Note>
