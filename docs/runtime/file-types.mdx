...
Import any file with `with { type: "bytes" }` to get its contents as a `Uint8Array`, per the [Import Bytes](https://github.com/tc39/proposal-import-bytes) proposal. Bun reads the file when the module is first imported; every import of the same file shares one `Uint8Array`, so treat it as read-only.
...
const { default: model } = await import("./model.onnx", { with: { type: "bytes" } });
```

To apply the loader by extension instead, use `--loader .bin:bytes` or `[loader]` in `bunfig.toml`. During bundling, Bun inlines the contents into the bundle as base64 and decodes them when the module is evaluated; `bun build --compile` embeds the file in the executable as it is.
