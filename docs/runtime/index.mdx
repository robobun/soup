...
If the `package.json` has a `"bun"` entry in its `"engines"` field, Bun checks its own version against that range before it runs the script, and fails with an error when the version is outside the range. See [Required Bun version](/pm/cli/install#required-bun-version).
