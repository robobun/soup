...
| `import.meta.glob`     | Import every file that matches a glob pattern, like Vite's `import.meta.glob`. `import.meta.glob("./pages/*.tsx")` becomes an object that maps each matched path to a function that imports it. See [Glob imports](/runtime/glob-imports).                                                                                    |
