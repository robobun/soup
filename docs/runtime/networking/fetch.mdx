...
The proxy can be an HTTP proxy (`http://`), an HTTPS proxy (`https://`) or a SOCKS5 proxy (`socks5://` or `socks5h://`, port `1080` by default). A SOCKS5 proxy is handed the hostname of the request and resolves it itself, and a username and password in the URL are used for SOCKS5 username/password authentication:
...
See the [proxy guide](/guides/http/proxy#socks5-proxies) for the error codes a SOCKS5 proxy can produce.

Without a `proxy` option, Bun reads `HTTP_PROXY`, `HTTPS_PROXY` and `ALL_PROXY` (upper or lower case) each time `fetch` runs, so changes to `process.env` apply to the next request. Bun uses `ALL_PROXY`, for `http:` and `https:` URLs alike, when the scheme-specific variable is unset. All three accept the same URLs as the `proxy` option. Bun ignores an `ALL_PROXY` value that names another kind of proxy, such as `socks4://`. curl reads `ALL_PROXY` too; Node.js does not. A request with the `unix` option never uses the proxy environment.
