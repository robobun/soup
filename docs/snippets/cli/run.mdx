...
<ParamField path="--watch-path" type="string">
  Also watch a file or directory that is not imported. Turns on <code>--watch</code> unless <code>--hot</code> is set.
  Can be repeated
</ParamField>

<ParamField path="--watch-exclude" type="string">
  Glob of files whose changes never restart or reload the process. Can be repeated
</ParamField>
