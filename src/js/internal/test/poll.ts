...
function poll(expect: unknown, fn: unknown, options: unknown) {
  ...
  if (options !== undefined) {
    ...
    } = options as { interval?: unknown; timeout?: unknown; message?: unknown };
    if (intervalOption !== undefined) {
      validateNumber(intervalOption, "options.interval", 0);
      interval = intervalOption as number;
    }
    if (timeoutOption !== undefined) {
      validateNumber(timeoutOption, "options.timeout", 0);
      timeout = timeoutOption as number;
    }
    if (messageOption !== undefined) {
      validateString(messageOption, "options.message");
      message = messageOption as string;
    }
  }
