...
impl Loader {
    ...
    /// The proxy for every target scheme. It commonly names a SOCKS proxy. A
    /// value with a scheme the HTTP client cannot speak (`socks4:`, ...) is
    /// left alone. A value with no scheme is an HTTP proxy, as for curl and
    /// for `HTTP_PROXY`: going direct instead would silently bypass the proxy,
    /// where a wrong guess fails loudly.
    fn all_proxy(&self) -> Option<&[u8]> {
        ...
        (url.protocol.is_empty() || url.has_http_like_protocol() || url.is_socks5())
            .then_some(value)
    }
